...
### Extracting release year and rating year from data
...
### Function that will be used to measure model performance
...
### Average rating of all movies in training set
mu <- mean(edx$rating)

#### Calculating movie effect
movies_df <- edx %>% group_by(movieId) %>% 
  summarise(movie_bias=sum(rating - mu)/(n() + 2.5 )) %>% 
...
validation_user_genres_df <- user_genres_df %>% select(-movieId) %>% 
  group_by(userId, genres) %>% summarise(genre_bias = mean(genre_bias))

validation_genre_biases <- validation_genre_ratings_df %>% 
  left_join(validation_user_genres_df, on=(c("userId", "genres"))) %>% 
  select(userId, movieId, genres, genre_bias)

validation_genre_biases <- validation_genre_biases %>% 
